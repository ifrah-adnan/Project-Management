...
export function AdminComponent({
  ...
  const handleViewDetails = (id: string) => async () => {
    try {
      ...
      router.push(`/projects?organizationId=${id}`);
    } catch (error) {
      console.error("Failed to set organization ID in session", error);
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredData.map((org) => (
            <Card
              key={org.id}
              className="w-full transform overflow-hidden rounded-lg border border-gray-200 bg-white from-white to-gray-100 shadow-md transition-all duration-300 ease-in-out hover:scale-[1.02] hover:border-blue-400 hover:bg-gradient-to-br hover:shadow-xl dark:bg-gray-800 dark:from-gray-800 dark:to-gray-700 dark:hover:border-blue-500"
            >
